...
import { Search, Menu, X, Loader2, Sparkles, Home, Bookmark, ChevronRight, ArrowLeft } from 'lucide-react';
...
export function Header() {
  ...
  // Click outside to close - handles both desktop and mobile
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (searchRef.current && !searchRef.current.contains(target)) {
        if (mobileSearchContainerRef.current && !mobileSearchContainerRef.current.contains(target)) {
          setShowResults(false);
        } else if (!mobileSearchContainerRef.current) {
          setShowResults(false);
        }
      }
    };
  ...
  useEffect(() => {
    if (isMobileSearchOpen && mobileSearchContainerRef.current) {
      ...
      animate(mobileSearchContainerRef.current, {
        translateY: [-20, 0],
        opacity: [0, 1],
        duration: 300,
        ease: 'outExpo'
  ...
  // Animate backdrop when menu opens
  useEffect(() => {
    if (isMobileMenuOpen && backdropRef.current) {
  ...
  // Animate back button when appearing
  useEffect(() => {
    if (!isHomePage && backButtonRef.current) {
      animate(backButtonRef.current, {
        translateX: [-10, 0],
        opacity: [0, 1],
        duration: 300,
        ease: 'outExpo'
  ...
  // Render search results - shared between mobile and desktop
    ...
    <>
      ...
      >
        <div className="container max-w-7xl h-18 py-3 sm:py-4 px-4 sm:px-6 flex items-center justify-between gap-3 sm:gap-6">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* Logo */}
            <Link href="/" className="group relative z-50" onClick={() => setIsMobileMenuOpen(false)}>
        ...
        {/* Mobile Search Bar - Inline in header, same approach as desktop */}
        {isMobileSearchOpen && (
          <div 
            ref={mobileSearchContainerRef}
            className="md:hidden border-t border-white/10 bg-background/95 backdrop-blur-xl px-4 pb-4"
            style={{ opacity: 0 }}
          >
      ...
      {isMobileMenuOpen && (
        <>
          ...
          <div 
            ref={drawerRef}
            className="fixed inset-y-0 right-0 z-[101] w-[80%] sm:w-[350px] md:hidden
